...
export const getConfigurationTool: RecapAITool = {
  ...
  handler: async (args: Record<string, unknown>) => {
    try {
      const typedArgs = args as ConfigurationArgs;

      if (typedArgs.key) {
        // Return specific key (but sanitize sensitive data)
        if (
          typedArgs.key.includes('token') ??
          typedArgs.key.includes('password')
        ) {
          throw new McpError(
        ...
        const value = config.get(typedArgs.key);
        return {
          success: true,
          data: {
            [typedArgs.key]: value,
      ...
      return {
        ...
        data: {
          ...
          metadata: {
            ...
            includeDefaults: typedArgs.includeDefaults !== false,
          },
