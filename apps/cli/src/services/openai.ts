...
export class OpenAIService {
  ...
  constructor(private config: Config | ConfigManager) {
    const apiKeyRaw = this.config.get('openai.token');
          ...
          if (enhanced && pr.enhancedData) {
            ...
            if (comments && comments.length > 0) {
              ...
              comments.slice(0, 2).forEach((c) => {
                const author =
                  'author' in c ? c.author : (c.user?.login ?? 'Unknown');
                const body =
                  'body' in c && typeof c.body === 'string'
                    ? c.body
                    : (c.body ?? '');
                const commentBody = body.substring(0, 100).replace(/\n/g, ' ');
                sections.push(
                  `    - ${author}: ${commentBody}${body.length > 100 ? '...' : ''}`
                );
              });
            }
          }
        ...
        data.github.issues.forEach((issue) => {
          ...
          if (issue.enhancedData) {
            ...
            if (
              ...
              issue.enhancedData.comments.slice(0, 2).forEach((c) => {
                const commentBody =
